...
CCPG::CCPG() {
   ...
   sigUpF = 0.0;
   sigDownF = 0.0;
   defaultModulation = false; /*Original value: true*/
   taskFrequency = 100.0;
   rateSaturation = 10.0;
   deltaOrigin.setZero();
   R.setIdentity();
    ...
    for (int i = 1; i <= subSteps; i++)
        {
            ...
            dXp(Z) = gamma * (1 - 4 * Xp(X)*Xp(X) / Ls / Ls - Xp(Z)*Xp(Z) / Hs / Hs) * Xp(Z) - Xp(X) * 2 * Hs / Ls * w_s; // Version OCTAVIO
