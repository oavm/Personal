...
private:
	...
	void maxPlusAlgebraModulation(); /* ADDED BY OCTAVIO */

	void changeModulation(); /* ADDED BY OCTAVIO */

	void whichModulation(); /* ADDED BY OCTAVIO */
	...
	OmegaGenerator omegaFrequency;
	Eigen::Matrix<double, 3, 8> eventsLog;
	Eigen::Matrix<double, 8, 1> xInitialMax;
	Eigen::Matrix<double, 1, 4> omegaVector;
	Eigen::ArrayXXd gaitPatternMax;
	Eigen::ArrayXXd timeDifference;
	int numberOfLegs; // Set number of legs of the platform
	double maxPlusTime,timeLeg;
	bool walkingFlag,maxPlusFlag;
// ===================   end of MaxPlus   ===================================
	...
	iit::dog::LegDataMap<bool> oldStanceLegs = false; /* OCTAVIO */
