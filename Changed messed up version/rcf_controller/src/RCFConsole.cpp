...
bool RCFController::addPromptCommands()
{
	...
	addFunction("maxPlusModulation", "Max-plus Modulation", &RCFController::maxPlusAlgebraModulation); /*ADDED BY OCTAVIO */
	addFunction("changeModulation", "Change Modulation", &RCFController::changeModulation); /*ADDED BY OCTAVIO */
	addFunction("whichModulation", "Which Modulation", &RCFController::whichModulation); /*ADDED BY OCTAVIO */
...
bool RCFController::ICTPLogic(const std::string& str) {
	if(str == "w"){
		if (stepFrequencyNew <= 2.95) {
			...
			stepLengthNew = forwVelNew * dutyF_New / stepFrequencyNew;
			/* Start Max-plus OCTAVIO */
			Eigen::Matrix<double, 1, 2> timeDifference;
			timeLeg = (1/(double)stepFrequencyNew)*(1 - 2*(1 - dutyF_New))*0.5;
			timeDifference << timeLeg,timeLeg;
      // xInitialMax << eventsLog.row(1);
			// schedule.set_gaitParametersMax(dutyF_New,stepFrequencyNew,
			// 														timeDifference,gaitPatternMax); /* ADDED BY OCTAVIO FOR MAXPLUS */
      // eventsLog = schedule.computefutureevents(eventsLog);
  ...
  /* Debugging OCTAVIO */
    ...
    cout << endl << "\n Time difference:  " << timeLeg;
    cout << endl << "\n Step frequency:  " << stepFrequencyNew;
    cout << endl << "\n Duty factor:  " << dutyF_New;
	}
