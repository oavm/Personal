							  ...
							  std::string robot)
{
	...
	deltaUff.resize(12);
	gaitPattern = 0; /* CHANGE BY OCTAVIO ORIGINAL VALUE: 0*/
	...
	/* INITIALIZATION OF STANCE LEGS FOR MAXPLUS UPDATE OCTAVIO */

	oldStanceLegs = stanceLegs;
...
/*	if(computeDeltaJumpHeight) {
	...
	if (attitudeFlag)
	{
	    ...
	    trunkController.setMinStanceLegs(2); /* Minimum number of stance legs OCTAVIO original value: 2*/
	...
	*/
	...
	maxPlusTime += 1/(double)taskServoRate;
    ...
    debugPush("CPG_couplingTermRH",(double)CPG[dog::RH].couplingTerm);
		...
		// Max-plus debugging variables OCTAVIO
		debugPush("stance_leg_LF",(double)stanceLegs[dog::LF]);
    debugPush("stance_leg_RF",(double)stanceLegs[dog::RF]);
    debugPush("stance_leg_LH",(double)stanceLegs[dog::LH]);
    debugPush("stance_leg_RH",(double)stanceLegs[dog::RH]);
...
void RCFController:: CPG_Modulation(dog::LegDataMap<rbd::Vector3d>& primitives,
    ...
    dog::LegDataMap<double> yaw_p(0.0);
		...
		/*ADDED BY OCTAVIO */
		if (maxPlusFlag){
			schedule.set_gaitParametersMax(dutyF_New,stepFrequencyNew,
																	timeDifference,gaitPatternMax);
			eventsLog = schedule.updatefutureevents(eventsLog,maxPlusTime);
			omegaVector = omegaFrequency.omegaGenerator(eventsLog,maxPlusTime);
			// xInitialMax << eventsLog.row(1);
			// eventsLog = schedule.initiallist(xInitialMax);
			// std::cout << "\nOmega: " << omegaVector;
		}
    ...
    for (int leg = dog::LF; leg <= dog::RH; leg++) {
        ...
        CPG[leg].couplingTerm = Kcoup * couplingTerm[leg];
				...
				/* ASSIGNING ANGULAR VELOCITY COMING FROM MAXPLUS ALGEBRA OCTAVIO */
				if (maxPlusFlag){
...
void RCFController::changeGaitPattern() {
	if (gaitPattern == 0) {
		...
		gaitPattern = 1;
		dutyF_New = 0.8;
		stepFrequencyNew = 1.5;
		Eigen::Matrix<double, 1, 4> timeDifference;
		Eigen::Matrix<double, 4, 1> gaitPatternMax;
		timeLeg = (1/stepFrequencyNew)*(1 - 2*(1 - dutyF_New))*0.25;
		...
		gaitPatternMax << 2,
											...
											1;
		schedule.set_gaitParametersMax(dutyF_New,stepFrequencyNew,
																timeDifference,gaitPatternMax); /* OCTAVIO */

		// VfX_New += 5*forwVelInc;
		// forwVelNew = sqrt(VfX_New * VfX_New + VfY_New * VfY_New);
		// stepLengthNew = forwVelNew * dutyF_New / stepFrequencyNew;
		...
		dutyF_New = 0.7;
		stepFrequencyNew = 1;
		Eigen::Matrix<double, 1, 2> timeDifference;
		Eigen::Matrix<double, 2, 2> gaitPatternMax;
		timeLeg = (1/stepFrequencyNew)*(1 - 2*(1 - dutyF_New))*0.5;
...
void RCFController::maxPlusAlgebraModulation() {
	maxPlusFlag = true;
	// omegaVector << 0,0,0,0;
	// eventsLog = schedule.initiallist(xInitialMax);
	CPG[dog::LF].Xp << 0.0001, 0.0, 0.0001;
	CPG[dog::RH].Xp << 0.0001, 0.0, 0.0001;
	CPG[dog::RF].Xp << 0.0001, 0.0, 0.0001;
	CPG[dog::LH].Xp << 0.0001, 0.0, 0.0001; /*CPG Initial Condition IMPORTANTMAX OCTAVIO*/
	for (int leg = dog::LF; leg <= dog::RH; leg++) {
		CPG[leg].setMaxPlusModulation(maxPlusFlag);
		CPG[leg].setDefaultAngularModulationFlag(!maxPlusFlag);
	}
	printf("Using max-plus algebra locomotion generator!!!\n");
}

void RCFController::changeModulation() {
	maxPlusFlag = false;
	CPG[dog::LF].Xp << 0.0001, 0.0, -0.0001;
	CPG[dog::RH].Xp = CPG[dog::LF].Xp;
	CPG[dog::RF].Xp << -0.0001, 0.0, 0.0001;
	CPG[dog::LH].Xp = CPG[dog::RF].Xp; /*CPG Initial Condition IMPORTANTMAX OCTAVIO*/
	for (int leg = dog::LF; leg <= dog::RH; leg++) {
		CPG[leg].setMaxPlusModulation(maxPlusFlag);
		CPG[leg].setDefaultAngularModulationFlag(!maxPlusFlag);
	}
	printf("Using coupling matrix syncrhonization!!!\n");
}

void RCFController::whichModulation() {
	std::cout << "\n Max-plus modulation is: \n" << maxPlusFlag;
}
...
void RCFController::walkStarting() {
	if (gaitPattern == 0) {
		...
		stepLengthNew = forwVelNew * dutyF_New / stepFrequencyNew;
		KcoupNew = 0.5;
		dutyF_New = 0.65; //Equivalent to Duty=5 Original value: 0.83 OCTAVIO;
		PsitNew = 0.0;
